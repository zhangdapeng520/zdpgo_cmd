...
func handleUnTar() {
	if file == "" {
		...
		if err != nil {
			Log.Error("un tar current dir error", "error", err, "directory", directory)
			return
		}
	} else {
		Log.Debug("un compress", "file", file)
		err := Tar.UnTarGzToSameDir(file)
		if err != nil {
			Log.Error("un tar file error", "error", err, "file", file)
			return
		}
	}
}
...
func handleTar() {
	...
	if directory == "./" {
		...
		if err != nil {
			Log.Error("compress child directory error", "error", err)
		}
		return
	}
	...
	if err != nil {
		Log.Error("compress directory to *.tar.gz error", "error", err, "directory", directory, "fileName", fileName)
		return
	}
	Log.Error("compress directory to *.tar.gz success", "directory", directory, "fileName", fileName)
}
